pipeline {
  ...
  environment {
    SERVICE_NAME = 'auth'
    NAMESPACE = 'auth-test'
    EUREKA_URL = 'http://discovery-service.auth-test.svc.cluster.local:8761/eureka'
  }
            ...
            container('maven') {
                  ...
                  string(credentialsId: 'STAGING_DATABASE_PASSWORD', variable: 'DATABASE_PASS')])
                {
                    ...
                    withSonarQubeEnv('SonarCloud') {
                        sh '''
                            mvn sonar:sonar \
                                -Dsonar.projectKey=My-Budget-Buddy_Budget-Buddy-AuthService \
                                -Dsonar.projectName=Budget-Buddy-AuthService \
